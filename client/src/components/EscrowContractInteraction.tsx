	...
	const onSubmitAggreement = React.useCallback(async (event: { preventDefault: () => void; }) => {
		...
		try {
			if (!!escrowContractRef.current) {
				...
				const allowance = await tokenContract.allowance(signer.getAddress(), escrowContractRef.current.address)
	
				// Calculate new allowance
				const newAllowance = allowance.add(initiatorSuppliedAmount);

				// Approve the new allowance
				const approveTx = await tokenContract.approve(escrowContractRef.current.address, newAllowance)
